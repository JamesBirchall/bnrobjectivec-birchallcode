...
int main(int argc, const char * argv[]) {
    @autoreleasepool {
        ...
        NSArray *top3ValuableHoldings = [portfolio topThreeByValue];
        
        for(JABForeignStockHolding *stock in top3ValuableHoldings){
            NSLog(@"Share %@ : Value: %.2f.\n", [stock symbol], [stock valueInDollars]);
        }
        
        NSLog(@"\n");
        
        NSArray *sortedBySymbol = [portfolio sortBySybmol];
        
        for(JABForeignStockHolding *stock in sortedBySymbol){
            NSLog(@"Share %@ : Value: %.2f.\n", [stock symbol], [stock valueInDollars]);
        }
